...
class ProfileScreen extends ConsumerStatefulWidget {
  ...
  final String? email;
  const ProfileScreen({
    super.key, required this.uid, this.displayName, this.email,
  });
...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  Future<void> _save() async {
    if (_nickCtrl.text.trim().isEmpty || _dob.isEmpty ||
        _gender.isEmpty || _genres.isEmpty) {
    ...
    if (age < 20) {
      ...
      try {
        final blocked = BlockedUser(
          ...
          birthday: _dob, blockedAt: DateTime.now().toIso8601String(),
        );
        await ref.read(authProvider.notifier).saveProfile(
          AppUser(
            uid: widget.uid, nickname: _nickCtrl.text.trim(),
            email: widget.email ?? '', birthday: _dob, gender: _gender,
            genres: _genres, tasteProfile: _sliders,
          ),
        );
  ...
  Widget build(BuildContext context) {
    final valid = _nickCtrl.text.trim().isNotEmpty && _dob.isNotEmpty &&
                  _gender.isNotEmpty && _genres.isNotEmpty;
    return Scaffold(
      appBar: nomigatorAppBar(title: 'プロフィール登録', showBack: false),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(20),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const SectionLabel('PROFILE SETUP'),
          const Text('お酒を楽しむための情報を教えてください。',
            style: TextStyle(fontSize: 13, color: kMuted, height: 1.75)),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 13, color: _dob.isEmpty ? kDim : kText,
                ),
              ),
            ),
          ),
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    color: on ? kGold.withOpacity(.12) : Colors.transparent,
                  ),
          ...
          const Text('直感で動かしてください。わからなければ中央のままでOKです。',
            style: TextStyle(fontSize: 11, color: Color(0xFF3a3028))),
  ...
  int _getAxisValue(String key) {
    switch (key) {
      case 'sweet':  return _sliders.sweet;
      case 'body':   return _sliders.body;
      case 'aroma':  return _sliders.aroma;
      case 'finish': return _sliders.finish;
      case 'kick':   return _sliders.kick;
      default:       return 0;
    }
  }
...
class _FieldLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Text(text, style: const TextStyle(
      fontSize: 9, letterSpacing: 2, color: kDim,
    )),
  );
}
