...
class _MyPageScreenState extends ConsumerState<MyPageScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: nomigatorAppBar(title: 'マイページ',
      ...
      body: ListView(
        ...
        children: [
          ...
          _Card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const _FL('プロフィール編集'),
            const _FL('ニックネーム'),
            TextField(controller: _nickCtrl,
              style: const TextStyle(color: kText),
              onChanged: (_) => setState(() {})),
            const SizedBox(height: 16),
            const _FL('紹介してほしいお酒のジャンル'),
            GenreSelector(selected: _genres,
            ...
            Row(children: [
              GoldButton(label: '変更を保存', loading: _saving,
                onPressed: _nickCtrl.text.trim().isNotEmpty && _genres.isNotEmpty
                  ? _save : null),
              if (_saved) ...[
          ...
          _Card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 14),
            const Text('レビュー1本につき1合もらえます。',
              style: TextStyle(fontSize: 11, color: kDim, height: 1.7)),
            const SizedBox(height: 16),
            const _FL('有料トークンを購入'),
            ...kTokenPlans.map((plan) => Padding(
              ...
              child: Row(
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(plan.desc,
                    style: const TextStyle(fontSize: 11, color: Color(0xFFc07a5a))),
                ]),
                GoldButton(
                  label: '購入', small: true,
                  onPressed: () async {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('${plan.desc}を購入しました！')));
                  },
                ),
              ]),
            )),
            const Text('※ 購入はStripe決済で処理されます（実装予定）。',
              style: TextStyle(fontSize: 10, color: Color(0xFF3a3028))),
          ])),
          const SizedBox(height: 14),
          ...
          // アクション
          _Card(child: Wrap(spacing: 12, runSpacing: 8, children: [
            GoldButton(label: 'ログアウト', outline: true,
              onPressed: () => ref.read(authProvider.notifier).signOut()),
            GoldButton(label: 'アカウントを削除する', outline: true, danger: true,
              onPressed: () => setState(() => _modal = 'delete')),
          ])),
...
class _TokenDisplay extends StatelessWidget {
  final int value; final String unit; final Color color;
  const _TokenDisplay(this.value, this.unit, this.color);
  @override
  Widget build(BuildContext context) => Column(children: [
    Text('$value', style: TextStyle(fontSize: 32, color: color,
...
本サービス「呑み語りN」は、お酒の好みに基づいたレコメンドおよびコミュニティ機能を提供します。
・20歳未満の方はご利用いただけません。
