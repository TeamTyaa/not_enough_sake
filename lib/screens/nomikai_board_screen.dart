// ============================================================
// screens/nomikai_board_screen.dart
// ============================================================
...
class _NomikaiBoard extends ConsumerState<NomikaiBoard> {
  ...
  double _dist(NomikaiPost post, TasteProfile myTaste) {
    ...
    final ds = t.sweet - myTaste.sweet, db = t.body - myTaste.body,
          da = t.aroma - myTaste.aroma, df = t.finish - myTaste.finish,
  ...
  Widget build(BuildContext context) {
    ...
    // ソート・フィルター
    var display = posts
      .where((p) => _dist(p, myTaste) <= kSimilarityThreshold)
      .toList();
      ...
      body: ListView(
        ...
        children: [
          ...
          Wrap(spacing: 6, runSpacing: 6, children:
            kSortOptions.map((opt) => _SortChip(
              label: opt.label, selected: _sort == opt.key,
              onTap: () => setState(() { _sort = opt.key; _visibleCount = kPageSize; }),
            )).toList(),
          ),
          ...
          // フィルター
          _FilterSection(
            city: _filterCity, minCap: _filterMinCap, maxCap: _filterMaxCap,
            onCityChanged:   (v) => setState(() { _filterCity = v;   _visibleCount = kPageSize; }),
            onMinCapChanged: (v) => setState(() { _filterMinCap = v; _visibleCount = kPageSize; }),
            onMaxCapChanged: (v) => setState(() { _filterMaxCap = v; _visibleCount = kPageSize; }),
            onClear: () => setState(() {
            ...
            ...visible.map((post) => _PostCard(
              ...
              onTap: () => Navigator.of(context).push(MaterialPageRoute(
                builder: (_) => PostDetailScreen(
                  post: post, myNick: user.nickname, myUid: user.uid, myTaste: myTaste,
                ),
              )),
            )),
            if (hasMore)
  ...
  void _showNewPostSheet(BuildContext context, AppUser user, TasteProfile myTaste) {
    showModalBottomSheet(
      ...
      builder: (_) => _NewPostSheet(
        authorNick: user.nickname, authorUid: user.uid, authorTaste: myTaste,
        onSubmit: (post) async {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        color: selected ? kGold.withOpacity(.12) : Colors.transparent,
      ),
      child: Text(label, style: TextStyle(
        fontSize: 10, letterSpacing: 1, color: selected ? kGold : kDim,
      )),
    ),
  );
}
...
class _FilterSection extends StatelessWidget {
  ...
  const _FilterSection({
    required this.city, required this.minCap, required this.maxCap,
    required this.onCityChanged, required this.onMinCapChanged,
    required this.onMaxCapChanged, required this.onClear,
  });
      ...
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Expanded(flex: 3, child: Column(children: [
          const Align(alignment: Alignment.centerLeft,
            child: Text('開催地', style: TextStyle(fontSize: 9, color: kDim))),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: kCard,
          border: Border.all(
            color: isConfirmed ? kGold.withOpacity(.35) : kBorder),
          borderRadius: BorderRadius.circular(3),
        ),
              ...
              children: [
                Expanded(child: Row(children: [
                  Text('● ${statusLabel[post.status] ?? post.status.name}',
                    style: TextStyle(fontSize: 10, color: statusColor[post.status] ?? kDim,
                      letterSpacing: 1)),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text(post.authorNick,
                  style: const TextStyle(fontSize: 14, color: kGold)),
...
class _InfoChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Text('$icon $label',
...
class _NewPostSheet extends StatefulWidget {
  ...
  const _NewPostSheet({
    required this.authorNick, required this.authorUid,
    required this.authorTaste, required this.onSubmit,
  });
...
class _NewPostSheetState extends State<_NewPostSheet> {
  ...
  bool get _valid => _titleCtrl.text.isNotEmpty && _date != null &&
    _placeCtrl.text.isNotEmpty && _budget != null &&
    _capCtrl.text.isNotEmpty && _minCtrl.text.isNotEmpty &&
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: SingleChildScrollView(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('NEW POST — 飲み会を開く',
            style: TextStyle(fontSize: 9, letterSpacing: 3, color: kDim)),
          IconButton(
        ...
        GestureDetector(
          onTap: () async {
            final picked = await showDatePicker(
              ...
              firstDate: _minDate, lastDate: _maxDate,
        ...
        const SizedBox(height: 7),
        Wrap(spacing: 6, runSpacing: 6, children:
          kBudgetOptions.map((b) => GestureDetector(
            onTap: () => setState(() => _budget = b),
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
              decoration: BoxDecoration(
                border: Border.all(color: _budget==b ? kGold : const Color(0xFF2a2420)),
                color: _budget==b ? kGold.withOpacity(.12) : Colors.transparent,
              ),
              child: Text(b, style: TextStyle(fontSize: 10,
                color: _budget==b ? kGold : kDim)),
            ),
          )).toList(),
        ),
        const SizedBox(height: 16),
        Row(children: [
          Expanded(child: _Input('定員（人）', _capCtrl, hint: '6',
            keyboard: TextInputType.number)),
          const SizedBox(width: 12),
          Expanded(child: _Input('確定必要人数', _minCtrl, hint: '3',
            keyboard: TextInputType.number)),
        ]),
...
class _Input extends StatelessWidget {
  ...
  const _Input(this.label, this.ctrl, {
