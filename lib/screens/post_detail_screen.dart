...
class PostDetailScreen extends ConsumerStatefulWidget {
  ...
  const PostDetailScreen({
    super.key, required this.post, required this.myNick,
    required this.myUid, required this.myTaste,
  });
...
class _PostDetailScreenState extends ConsumerState<PostDetailScreen> {
  ...
  Future<void> _submitComment() async {
    ...
    try {
      final c = PostComment(
        id: '', nick: widget.myNick, uid: widget.myUid,
  ...
  Future<void> _addIntent() async {
    try {
      ...
      final newStatus = newIntents.length >= _post.minAttendees &&
                        _post.status == PostStatus.open
        ? PostStatus.confirmed : _post.status;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('参加意志の登録に失敗しました')));
    }
  }
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: kCard,
            border: Border.all(
              color: _post.status == PostStatus.confirmed
                ? kGold.withOpacity(.35) : kBorder),
            borderRadius: BorderRadius.circular(3),
          ),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            if (_post.genre != null) ...[
              CategoryTag(_post.genre!), const SizedBox(height: 10),
            ],
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              GestureDetector(
                onTap: () => _navigateToUser(_post.authorNick, _post.authorUid),
                child: Text(_post.authorNick, style: const TextStyle(
                  fontSize: 15, color: kGold,
            ...
            Wrap(spacing: 16, runSpacing: 6, children: [
              _IC('📅', _post.date), _IC('📍', _post.place),
              _IC('💴', _post.budget),
              _IC('👥', '${_post.intents.length}/${_post.capacity}人'),
            ]),
            const SizedBox(height: 8),
            Text('開催確定条件: 参加意志 ${_post.minAttendees}人以上',
            ...
            Wrap(spacing: 8, runSpacing: 8, children: [
              ...
              if (!_isAuthor && _post.status == PostStatus.open && _hasIntent)
                GoldButton(label: '参加意志をキャンセル', small: true, outline: true,
                  danger: true, onPressed: _cancelIntent),
              if (_canRate)
                GoldButton(label: '評価を送る', small: true, outline: true,
              ...
              if (!_isAuthor)
                TextButton(
                  ...
                  child: const Text('⚑ 通報',
                    style: TextStyle(fontSize: 10, color: Color(0xFF5a3a3a))),
                ),
            ]),
          ]),
        ),
        ...
        if (_post.intents.isNotEmpty) ...[
          ...
          Wrap(spacing: 8, runSpacing: 8, children:
            _post.intents.map((i) => GestureDetector(
          ...
          ..._post.ratings.map((r) => Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                CategoryTag(r.role == 'host' ? '開催者' : '参加者',
                  color: r.role == 'host' ? kGold : kGreen),
                const SizedBox(width: 8),
        ...
        if (_err.isNotEmpty)
          ErrorBanner(message: _err, onDismiss: () => setState(() => _err = '')),
                ...
                onPressed: stars > 0 && commentCtrl.text.isNotEmpty
                  ? () async {
                      final rating = PostRating(
                        fromNick: widget.myNick, fromUid: widget.myUid,
                        toNick: _isAuthor ? '参加者' : _post.authorNick,
                        role: _isAuthor ? 'host' : 'guest',
                        stars: stars, comment: commentCtrl.text.trim(),
                      );
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setS) => Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min,
            ...
            ...kReportReasons.map((r) => GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: reason==r ? kGold.withOpacity(.1) : Colors.transparent,
                ),
                child: Text(reason==r ? '✓ $r' : r,
                  style: TextStyle(fontSize: 12,
                    color: reason==r ? kGold : kMuted)),
              ),
            )),
            const SizedBox(height: 8),
            TextField(
              controller: detailCtrl, maxLines: 2,
              style: const TextStyle(color: kText, fontSize: 13),
            ...
            Row(children: [
              Expanded(child: GoldButton(
                label: '通報する', danger: true,
                onPressed: reason != null
                  ? () async {
                      await MockDb.addReport(ReportData(
                        postId: _post.id, authorNick: _post.authorNick,
                        reporterUid: widget.myUid, reporterNick: widget.myNick,
                        reason: reason!, detail: detailCtrl.text,
...
class _CommentTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      GestureDetector(
        onTap: onTapName,
        child: Text(comment.nick, style: const TextStyle(
